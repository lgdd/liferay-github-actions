...
func createOrEditPullRequest(mainBranchName, title, body string) {
	...
	if err != nil {
		...
		fmt.Println("Run pr create " + upgradeBranchName + " --base " + mainBranchName )
		stdoutBuffer, stderrBuffer, err := gh.Exec("pr", "create", upgradeBranchName, "--base", mainBranchName, "-t", title, "-b", body)
